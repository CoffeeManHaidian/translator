...
from platforms.windows.input_sender import WindowsCopyKeySender


class WindowsClipboardTextCapture(TextCapture):
    """模拟 Ctrl+C，异步读取所选文字并恢复原剪贴板。"""

    def __init__(
        self,
        clipboard: QClipboard | None = None,
        input_sender: WindowsCopyKeySender | None = None,
        timeout_ms: int = 1_000,
        copy_delay_ms: int = 80,
        parent: QObject | None = None,
    ) -> None:
        super().__init__(parent)
        self._clipboard = clipboard or QGuiApplication.clipboard()
        self._input_sender = input_sender or WindowsCopyKeySender()
    ...
    def capture_selected_text(self) -> None:
        ...
        # 写入标记时保持非捕获状态，避免处理自己的 dataChanged。
